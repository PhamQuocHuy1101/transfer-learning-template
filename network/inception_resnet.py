...
class Inception_resnet(nn.Module):
    ...
    def __init__(self, in_channel, n_class, n_block_A, n_block_B, n_block_C, drop_r = 0.2):
        ...
        self.inception_A = nn.Sequential(*[Inception_A(384) for _ in range(n_block_A)]) # 35, 384 -> 35, 384
        self.reduce_A = Reduction_A(384) # 35, 384 -> 17, 1152
        self.inception_B = nn.Sequential(*[Inception_B(1152) for _ in range(n_block_B)]) # 17, 1152 -> 17, 1152
        self.reduce_B = Reduction_B(1152) # 17, 1152 -> 8, 2144
        self.inception_C = nn.Sequential(*[Inception_C(2144) for _ in range(n_block_C)]) # 8, 2144 -> 8, 2144
